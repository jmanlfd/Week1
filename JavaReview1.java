...
public class JavaReview1 {
	...
	public static void main(String[] args) {
		int[] ints = { 39, 46, 10, 37, 33, 4, 30, 26, 14, 19, 29, 6, 43, 8, 35, 50, 13, 25, 17, 48, 28, 3, 41, 34, 36,
		...
		for (int i = 0; i < ints.length; ++i) {
			if (ints[i] == number) {
				System.out.println("The number " + number + " was found at index " + i + " in the array.");
				break;
			}
		}
		...
		for (int i = 0; i < payments.length; ++i) {
			...
			System.out.print(customers[i] + " " + payments[i][0] + " ");
			for (int j = 1; j < 13; ++j) {
				if (payments[i][j] == 0)
					++missedPayments;
